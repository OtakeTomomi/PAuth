...
def load_frank_data(user_n, document, doc_list):
    ...
    df_read_fM_doc = pd.read_csv("featMat_fu_doc/featMat_fu_doc{0}_{1}.csv".format(user_n, doc_list[document]), sep=",", index_col=0)
    # indexのリセット
...
for user_n in range(1, 42, 1):
    ...
    df_read_fM = pd.read_csv(f'featMat_fu/featMat_fu{user_n}.csv', header=None, index_col=0)
    ...
    for document in range(len(doc_list)):
        ...
        # FC_docファイルがなければ作成
        os.makedirs('feature_calc_doc', exist_ok=True)
        # 各ユーザの各ドキュメントごとに書き出しを行う
        df.to_csv('feature_calc_doc/fc{0}_{1}_doc.csv'.format(user_n, doc_list[document]))
